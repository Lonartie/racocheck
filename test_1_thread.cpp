...
int main() {
   ...
   std::cout << "=== Testing Multi-threaded Mode with 1 thread ===\n";
   ...
   try {
      auto result = check()
             ...
             .depth_limit(3)  // Reduced for debugging
             .ignore_depth_limit_warning()
             .deadlock_timeout(std::chrono::milliseconds(100))
             .info_stream_to(std::cout)
             .error_stream_to(std::cerr)
             .continue_on_error()
             .num_threads(1)  // Use multi-threaded code path but with 1 thread
             .run();
             
      std::cout << "Multi-threaded (1 thread) result: " << (result ? "PASS" : "FAIL") << "\n";
